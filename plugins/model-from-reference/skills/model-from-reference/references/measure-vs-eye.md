...
| **Two tolerances, not one** | The line between them is **what is being compared** (see [phases.md](phases.md), M2, item 3): the model against the reference is always the form tolerance; two readings of the same kind against each other — markup against reference, one built end cap against another, one markup level against its neighbour — are the measurement tolerance. **Measurement tolerance** is the precision with which the reference was read (see [work-rules.md](work-rules.md), R6, item 8). **Form tolerance** is how far the model is entitled to diverge from the reference; it is **wider**, and it is set here, before building. A two-dimensional reference does not transfer into three dimensions one to one: it has its own asymmetry, its own capture distortions, its own contour thickness. Holding the form to the measurement tolerance means driving it to the error of the source |
